...
parms <- list(canopy_threshold = canopy_threshold, K = K, r = r, g = r)
...
Graph the results
```{r fig.width=7}
carbon_graph <- ggplot(data = as.data.frame(results), aes(x = year, y = carbon)) +
  geom_point() +
  labs(x = "Year", y = "Forest Size (Carbon)",
       title = "Modeled Forest Size",
       subtitle = paste0("initial size = ", Cinitial, " kg C, r = ", r, " kg/year, g = ", g, "kg/year, canopy threshold = ", canopy_threshold, "kg C, carrying capacity = ", K, " kg C"))
carbon_graph
...
ggplot(tmp, aes(x = metric, y = value, col = metric)) +
  geom_box()
```
